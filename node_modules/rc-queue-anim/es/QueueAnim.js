  ...
  this.getChildrenToRender = function (child) {
    ...
    if (i >= 0) {
      ...
      if (_this5.tweenToEnter[key] && !forcedReplay) {
        // 如果是已进入的，将直接返回标签。。
        return createElement(TweenOne, { key: key, component: child.type, forcedJudg: forcedJudg, componentProps: child.props });
      }
    }
    var paused = _this5.keysToEnterPaused[key] && !_this5.keysToLeave.indexOf(key) >= 0;
    animation = paused ? null : animation;
    var tag = createElement(TweenOne, { key: key, component: child.type, id: key, forcedJudg: forcedJudg, componentProps: child.props, animation: animation });
